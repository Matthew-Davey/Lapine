...
type DeserializerBuilder() =
    let run state (f: ReadOnlyMemory<uint8> -> ReadOnlyMemory<uint8> * 'T) = f state
    ...
    member _.Return result =
        fun (state: ReadOnlyMemory<uint8>) -> (state, result)
...
let readBoolean (buffer: ReadOnlyMemory<uint8>) = (buffer[1..], buffer.Span[0] > 0uy)

let readChar (buffer: ReadOnlyMemory<uint8>) = (buffer[1..], char (buffer.Span[0]))

let readBytes (count: uint16) (buffer: ReadOnlyMemory<uint8>) =
    (buffer[int count ..], buffer[.. int count])

let readUInt8 (buffer: ReadOnlyMemory<uint8>) = (buffer[1..], buffer.Span[0])

let readInt8 (buffer: ReadOnlyMemory<uint8>) = (buffer[1..], int8 buffer.Span[0])
...
let rec readFieldValue =
    deserialize {
        let box (buffer, result) = buffer, box result

        match! readChar with
        | 't' -> return! readBoolean >> box
        | 'b' -> return! readInt8 >> box
        | 'B' -> return! readUInt8 >> box
        | 's' -> return! readUInt16BE >> box
        | 'u' -> return! readUInt16BE >> box
        | 'I' -> return! readInt32BE >> box
        | 'i' -> return! readUInt32BE >> box
        | 'L' -> return! readInt64BE >> box
        | 'l' -> return! readUInt64BE >> box
        | 'f' -> return! readSingle >> box
        | 'd' -> return! readDouble >> box
        | 'D' -> return! readDecimal >> box
        | 'S' -> return! readLongString >> box
        | 'x' -> return! readByteArray >> box
        | 'A' -> return! readFieldArray >> box
        | 'T' -> return! readTimestamp >> box
        | 'V' -> return DBNull.Value
        | 'F' -> return! readFieldTable >> box
...
let writeBoolean (value: bool) (writer: IBufferWriter<uint8>) =
    ...
    buffer[0] <- if value then 1uy else 0uy
    writer.Advance 1
    writer
...
let rec writeFieldValue (value: obj) =
    ...
    | :? array<uint8> as value -> writeChar 'x' >> writeBytes (ReadOnlyMemory.op_Implicit value)
    ...
    | :? Map<string, bool> as value -> writeChar 'F' >> writeFieldTable (Map.map (fun _ -> box) value)
    | null -> writeChar 'V'
    | _ -> failwith "Unsupported field value"

and writeFieldArray (value: obj seq) = writeUInt8 0uy
...
and writeFieldTable (value: Map<string, obj>) =
    ...
    let rows =
        Map.fold
            (fun accumulator field value -> accumulator |> writeShortString field |> writeFieldValue value)
            buffer
            value
