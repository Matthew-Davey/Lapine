namespace Lapine.Agents {
    ...
    public class FrameHandlerAgentTests : Faker {
        ...
        [Fact]
        public void HandlesBasicCancelOkMethodFrame() {
            var inbound = new BasicCancelOk(
                consumerTag: Random.Word()
            );
            _context.Send(_subject, new FrameReceived(RawFrame.Wrap(channel: 0, inbound)));

            if (_messageReceivedSignal.Wait(timeout: TimeSpan.FromMilliseconds(100))) {
                var outbound = _outboundMessage as BasicCancelOk;

                Assert.Equal(expected: inbound.ConsumerTag, actual: outbound.ConsumerTag);
            }
            else {
                // No `BasicCancelOk` command was handled within 100 millis...
                throw new TimeoutException("Timeout occurred before command was handled");
            }
        }
        ...
        [Fact]
        public void HandlesBasicDeliverMethodFrame() {
            var inbound = new BasicDeliver(
                consumerTag : Random.Word(),
            ...
            if (_messageReceivedSignal.Wait(timeout: TimeSpan.FromMilliseconds(100))) {
                var outbound = _outboundMessage as BasicDeliver;
                ...
                // No `BasicDeliver` command was handled within 100 millis...
                throw new TimeoutException("Timeout occurred before command was handled");
            }
        }
        ...
        [Fact]
        public void HandlesBasicRecoverOkMethodFrame() {
            _context.Send(_subject, new FrameReceived(RawFrame.Wrap(channel: 0, new BasicRecoverOk())));

            if (_messageReceivedSignal.Wait(timeout: TimeSpan.FromMilliseconds(100))) {
                Assert.True(_messageReceivedSignal.IsSet);
            }
            else {
                // No `BasicRecoverOk` command was handled within 100 millis...
                throw new TimeoutException("Timeout occurred before command was handled");
            }
        }

        [Fact]
        public void HandlesTransactionSelectOkMethodFrame() {
            _context.Send(_subject, new FrameReceived(RawFrame.Wrap(channel: 0, new TransactionSelectOk())));

            if (_messageReceivedSignal.Wait(timeout: TimeSpan.FromMilliseconds(100))) {
                Assert.True(_messageReceivedSignal.IsSet);
            }
            else {
                // No `TransactionSelectOk` command was handled within 100 millis...
                throw new TimeoutException("Timeout occurred before command was handled");
            }
        }

        [Fact]
        public void HandlesTransactionCommitOkMethodFrame() {
            _context.Send(_subject, new FrameReceived(RawFrame.Wrap(channel: 0, new TransactionCommitOk())));

            if (_messageReceivedSignal.Wait(timeout: TimeSpan.FromMilliseconds(100))) {
                Assert.True(_messageReceivedSignal.IsSet);
            }
            else {
                // No `TransactionCommitOk` command was handled within 100 millis...
                throw new TimeoutException("Timeout occurred before command was handled");
            }
        }

        [Fact]
        public void HandlesTransactionRollbackOkMethodFrame() {
            _context.Send(_subject, new FrameReceived(RawFrame.Wrap(channel: 0, new TransactionRollbackOk())));

            if (_messageReceivedSignal.Wait(timeout: TimeSpan.FromMilliseconds(100))) {
                Assert.True(_messageReceivedSignal.IsSet);
            }
            else {
                // No `TransactionRollbackOk` command was handled within 100 millis...
                throw new TimeoutException("Timeout occurred before command was handled");
            }
        }
    }
